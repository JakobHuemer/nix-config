{
  ...
  options.zsh.consul_http_token_path = lib.mkOption {
    type = lib.types.nullOr lib.types.str;
    default = null;
  };
  options.zsh.nomad_token_path = lib.mkOption {
    type = lib.types.nullOr lib.types.str;
    default = null;
  };
    ...
    programs = {
      zsh = {
        ...
        initContent = ''
          ...
          if [[ -r "${config.zsh.consul_http_token_path}" ]]; then
            export \
              CONSUL_HTTP_TOKEN="$(cat ${config.zsh.consul_http_token_path})"
          fi

          if [[ -r "${config.zsh.nomad_token_path}" ]]; then
            export \
              NOMAD_TOKEN="$(cat ${config.zsh.nomad_token_path})"
          fi
